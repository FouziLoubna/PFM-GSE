...
export class EtudiantService {
  ...
  private headersProvider(token: string): HttpHeaders {
  ...
  public save(): Observable<Etudiant> {
    const headers = this.headersProvider(this.authService.retrieveUser().token);
    return this.http.post<Etudiant>(this.API, this.selectedEtudiant, { headers });
  }

 // public update(passwordChanged: boolean): Observable<number> {
   // const headers = this.headersProvider(this.authService.retrieveUser().token);
    //return this.http.put<number>(`${this.API}update/${passwordChanged}`, this.selectedEtudiant, { headers });
  //}
